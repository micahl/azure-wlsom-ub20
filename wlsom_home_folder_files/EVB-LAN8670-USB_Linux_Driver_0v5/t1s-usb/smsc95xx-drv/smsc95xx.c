...
static int smsc95xx_phy_update_flowcontrol(struct usbnet *dev)
{
	...
	lcladv = phy_read(dev->net->phydev, MII_ADVERTISE);
	if (lcladv < 0)
		return lcladv;

	rmtadv = phy_read(dev->net->phydev, MII_LPA);
	if (rmtadv < 0)
		return rmtadv;
...
static int get_mdix_status(struct net_device *net)
{
	...
	buf = smsc95xx_mdio_read_nopm(dev, SPECIAL_CTRL_STS);
	if (buf < 0)
		return buf;
...
static void set_mdix_status(struct net_device *net, __u8 mdix_ctrl)
{
	...
	if ((pdata->chip_id == ID_REV_CHIP_ID_9500A_) ||
		...
		buf = smsc95xx_mdio_read_nopm(dev, PHY_EDPD_CONFIG);
		if (buf < 0)
			return;
		buf |= PHY_EDPD_CONFIG_EXT_CROSSOVER_;
		smsc95xx_mdio_write_nopm(dev, PHY_EDPD_CONFIG, buf);
	}
	...
	if (mdix_ctrl == ETH_TP_MDI) {
		...
		smsc95xx_mdio_write_nopm(dev, SPECIAL_CTRL_STS, buf);
...
static int smsc95xx_ndo_open(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
	int ret;
	...
	phy_init_hw(net->phydev);
	phy_start(net->phydev);

	return usbnet_open(net);
}

static int smsc95xx_ndo_stop(struct net_device *net)
{
	if (net->phydev) {
		if (phy_is_started(net->phydev))
			phy_stop(net->phydev);
	}
...
static const struct net_device_ops smsc95xx_netdev_ops = {
	.ndo_open		= smsc95xx_ndo_open,
	.ndo_stop		= smsc95xx_ndo_stop,
	.ndo_start_xmit		= usbnet_start_xmit,
	.ndo_tx_timeout		= usbnet_tx_timeout,
	.ndo_change_mtu		= usbnet_change_mtu,
	.ndo_get_stats64	= dev_get_stats64,
